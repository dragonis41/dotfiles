...
function configure_yubikey(){
    ...
    echo "To continue, execute the following commands :"
...
function prompt_packages(){
    read -p "Install base packages before restoring configuration files ? [yn] " yn
    case $yn in
...
function prompt_extra_packages(){
    read -p "Install extra packages before restoring configuration files ? [yn] " yn
    case $yn in
...
function prompt_ohmyzsh(){
    read -p "Install Oh My Zsh with plugins before restoring configuration files ? Warning : It will delete current installation [yn]  " yn
    case $yn in
