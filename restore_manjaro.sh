...
function install_packages(){
    ...
    sudo -H -u "$SUDO_USER" bash -c 'yay --noconfirm --answerclean All --answerdiff None --answeredit None --cleanafter --removemake --sudoloop -S bind linux66 linux66-headers mkinitcpio-firmware autojump fprintd fd jq dialog gum noto-fonts-emoji mtr nano-syntax-highlighting'
    if (($? != 0)); then
